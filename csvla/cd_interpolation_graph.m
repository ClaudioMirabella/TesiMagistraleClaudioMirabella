function OutputArg1 = cd_interpolation_graph(Cd, Cd_interp, XI, YI)

% function OutputArg1 = cd_interpolation_graph(Cd_interp, XI, YI)
...
OutputArg1 = figure;
% plot3(x, y(1)*ones(length(x), 1), Cd(:,1), '-b.', 'MarkerSize', 1, 'LineWidth', 0.5)
plot3(XI(:,1), YI(:,1), Cd(:,1), '-b.', 'MarkerSize', 1, 'LineWidth', 2)
hold on

    for i = 2:length(YI(1,:))
        % plot3(x, y(i)*ones(length(x), 1), Cd(:,i), '-b.', 'MarkerSize', 1, 'LineWidth', 0.5)
        plot3(XI(:,i), YI(:,i), Cd(:,i), '-b.', 'MarkerSize', 1, 'LineWidth', 2)
    end
...
zlabel("$C_d = C_d(y)$", "Interpreter", "latex")
