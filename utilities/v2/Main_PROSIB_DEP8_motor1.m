...
ac.name = 'PROSIB_DEP8_motor1_cruise_propON';      % aircraft name
...
% Winglets are obtained as very small panels with a big dihedral.
...
if propON == 1                                        % enable all rotors
    ...
    % (positive value = clockwise front view --> Outboard Up on right wing)
...
% Since there is no symmetry, in prop-on analyses the propulsors must be
% mirrored, otherwise thrust effects will be asymmetric!
if propON == 1                                        % enable all rotors SYMMETRIC THRUST
    set.NumberOfRotors = 2*set.NumberOfRotors;       % set geometric and aerodyanmic data arrays
    set.xloc = [fliplr(set.xloc), set.xloc];          % prop global X location
    set.yloc = [fliplr(-set.yloc), set.yloc];         % prop global Y location
    set.zloc = [fliplr(set.zloc), set.zloc];          % prop global Z location
    set.xdir = [fliplr(set.xdir), set.xdir];          % prop X unit normal
    set.ydir = [fliplr(-set.ydir), set.ydir];         % prop Y unit normal
    set.zdir = [fliplr(set.zdir), set.zdir];          % prop Z unit normal
    set.dia = [fliplr(set.dia), set.dia];             % prop diameter
    set.hub = [fliplr(set.hub), set.hub];             % prop hub
    
    % set RPM to zero to disable that rotor into VSPaero
    set.rpm = [fliplr(-set.rpm), set.rpm]; % counterotating props
    set.ct = [fliplr(set.ct), set.ct];
    set.cp = [fliplr(set.cp), set.cp];
end
