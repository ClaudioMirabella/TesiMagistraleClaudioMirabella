...
ac.name = 'BWB_test';      % aircraft name
...
ac.wing.thickchord  = [0.18 0.10 0.09];      % thickness of each airfoil
ac.wing.twist       = [3 1 -5];                  % twist angles
...
ac.wing.span        = [10.0 20.0];          % span(s)
ac.wing.ctip        = [15.0 4.0];           % tip chord(s)
ac.wing.croot       = [40.0 ac.wing.ctip(1)];  % root chord(s)
ac.wing.sweep       = [65 45];                 % sweep angle(s)
...
ac.wing.blend.InLEMode = [NaN, 2, 2]; % leading edge inboard blending type
...
ac.wing.blend.OutLEMode = [1, 2, NaN];  % leading edge outboard blending type
...
ac.wing.blend.InTEMode = [NaN, 3, 3]; % trailing edge inboard blending type
...
ac.wing.blend.OutTEMode = [1, 3, NaN];  % trailing edge outboard blending type
...
dos(vspscript_exe);
% [status, data] = dos(vspscript_exe);
% set.cd0 = str2double(data); % works if in data there is a scalar only
...
vspaero_exe     = ['vspaero -omp ' num2str(num_cpu)...   % command
    ' ' ac.name '_DegenGeom'];
try
    status = dos(vspaero_exe);                                % executing command
    if status ~= 0
...
CalcParasiteDrag(ac,set.Vinf,set.h)

parasitedrag_exe = ['vspscript.exe -script ' ac.name '_ParasiteDrag.vspscript'];
[status, data] = dos(parasitedrag_exe);
set.cd0 = str2double(data); % works if in data there is a scalar only

disp(['CD0 = ', num2str(set.cd0,'%.4f')])
